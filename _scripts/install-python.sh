...
if is_dry_run; then
    ...
    run_shell 'LATEST_PYTHON_VERSION=$(pyenv install --list | grep -E "^[[:space:]]*[0-9][0-9.]*[0-9][[:space:]]*$" | tail -1 | tr -d " ")'
...
LATEST_PYTHON_VERSION=$(pyenv install --list | grep -E '^[[:space:]]*[0-9][0-9.]*[0-9][[:space:]]*$' | tail -1 | tr -d ' ')
[[ -n "$LATEST_PYTHON_VERSION" ]] || die "pyenv did not report a stable Python version to install."
