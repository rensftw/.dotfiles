...
path=(
  ...
  "$BREW_PREFIX/opt/lua-language-server/bin"
  "$HOME/.local/bin" # For Claude Code binary
  "$HOME/.bin"
  "$HOME"
  "${path[@]}"
)
...
# History settings
# Docs: man zshoptions
# Kept above all source/eval lines so a failure there can never skip these options.
...
if [[ -s "$NVM_DIR/alias/default" ]]; then
  ...
  path=("$NVM_DIR/versions/node/v${_v#v}/bin" "${path[@]}")
  export PATH
  unset _v
fi
